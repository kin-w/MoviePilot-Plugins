...
try:
    ...
    # 旧版宿主（V1）可能未导出 ServiceInfo；该符号仅在类型注解中使用
    ServiceInfo = Any
...
class CrossSeedSkipVerify(_PluginBase):
    # 插件名称
    plugin_name = "蛙辅·跳校版"
    # 插件描述
    plugin_desc = "支持NexusPHP站点自动辅种，基于“青蛙辅种助手”修改并新增跳过校验功能。"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "cross_seed_sv_"
    # 加载顺序
    plugin_order = 17
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._token = config.get("token") or ""  # passkey格式  青蛙:xxxxxx,站点名称:xxxxxxx

            self._downloaders = config.get("downloaders")
            self._torrentpath = config.get("torrentpath") or ""  # 种子路径和下载器对应  /qb,/tr
            self._torrentpaths = self._torrentpath.strip().split(",") if self._torrentpath else []
        ...
        if self.get_state():
            ...
            if self._cron:
                ...
                for trigger in triggers:
                    ret_jobs.append({
                        "id": f"CrossSeedSV|{trigger.hour}:{trigger.minute}",
                        "name": "蛙辅·跳校版",
                        "trigger": "cron",
                        "func": self.auto_seed,
                        "kwargs": {
            ...
            logger.warn("蛙辅·跳校版插件参数不全，定时任务未正常启动")
        return []
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': '1. 周期建议间隔>1天，留空则每天2-7点随机一次； '
                                                    '2. 站点名需与上方选项一致，仅辅种选中站点，passkey可多条； '
                                                    '3. 请勿与IYUU对相同站点同时启用； '
                                                    '4. 访问 {站点域名}/api/pieces-hash 返回405则大概率支持。'
                                        }
                                    }
                                ]
                            }
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': '【进阶】接口偶发失败时，可在 passkey 后加 :秒数 设置该站点请求间隔（整数，默认1秒），示例：站点名称:Passkey:3'
                                        }
                                    }
                                ]
                            }
    ...
    def auto_seed(self):
        ...
        for idx, service in enumerate(self.service_infos.values()):
            ...
            for torrent in torrents:
                ...
                # 获取种子文件路径
                if idx >= len(self._torrentpaths):
                    logger.error(f"下载器 {downloader} 没有对应的种子文件目录配置，跳过该种子")
                    continue
